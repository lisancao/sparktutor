"""
Batch vs Streaming Transaction Count — Starter Code

Implement transaction_count_batch and transaction_count_streaming_logic.
Both produce transaction count per user_id, ordered by count descending.
The streaming logic uses the same transformations — only the source differs.
"""
...
def transaction_count_streaming_logic(spark, input_df):
    ...
    In production, input_df would come from readStream. Here we pass a batch."""
    return None
