...
Simulates streaming micro-batches with union and aggregation.
"""
...
def streaming_revenue_by_product(spark, batches: list) -> DataFrame:
    """
    Simulate streaming: union all batches, aggregate sum(revenue) by product_id.
    Returns DataFrame: product_id, total_revenue.
    """
    combined = batches[0]
    for b in batches[1:]:
        combined = combined.union(b)
    return combined.groupBy("product_id").agg(f.sum("revenue").alias("total_revenue"))
