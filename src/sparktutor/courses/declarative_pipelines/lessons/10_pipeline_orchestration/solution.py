"""
Pipeline Orchestration (Solution)

Full ShopStream pipeline with error handling and run reporting.
"""
    ...
    def run(self):
        ...
        for name in order:
            try:
                ...
                count = df.count()
                report.append({"step": name, "status": "ok", "rows": count, "error": None})
            except Exception as e:
                report.append({"step": name, "status": "failed", "rows": 0, "error": str(e)})
                raise
        return report
...
def run_shopstream_pipeline(spark, clicks_path):
    ...
    @pipeline.materialized_view("bronze_clicks")
    def bronze():
        return bronze_clicks(spark, clicks_path)

    @pipeline.materialized_view("silver_clicks")
    def silver():
        bronze_df = spark.table("bronze_clicks")
        return silver_clicks(spark, bronze_df)

    @pipeline.materialized_view("gold_daily")
    def gold():
        silver_df = spark.table("silver_clicks")
        return (silver_df
